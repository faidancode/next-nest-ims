import { Customer, CustomerListResponse } from "@/types/customer.type";
...
export async function getCustomers(
  ...
  return {
    data: unwrapEnvelope(envelope, "Failed to fetch Customers"),
    meta: envelope.meta,
  };
}
...
export async function updateCustomer(
  ...
  const envelope = await apiRequest<Customer>(`/customers/${id}`, data, {
    method: "PATCH",
  });
...
export async function deleteCustomer(id: string): Promise<void> {
  const envelope = await apiRequest<void>(`/customers/${id}`, undefined, {
    method: "DELETE",
  });
